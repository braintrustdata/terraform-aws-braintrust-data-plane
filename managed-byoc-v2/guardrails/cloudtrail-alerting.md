...
Raise them to High when session attribution, target scope, or change correlation
does not match the approved operation.
...
- `DisableRule`, `EnableRule`: exact job and expected resume time. Alert when a
  paused job remains disabled after its incident window.
...
3. Determine whether protected data, credentials, networking, or retention was
   affected.
4. Restore intended configuration through the reviewed deployment workflow.
5. Reconcile legitimate emergency changes into Terraform and close the linked

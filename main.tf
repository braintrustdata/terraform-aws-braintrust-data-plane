...
module "loop_runtime_ecs" {
  ...
  create_gateway_ingress_rule   = local.create_ai_gateway
  gateway_alb_security_group_id = local.create_ai_gateway ? module.gateway_alb[0].gateway_alb_security_group_id : null
  service_dependency_ids = concat(
    module.loop_runtime_sandbox_aws_microvm[0].private_endpoint_dependency_ids,

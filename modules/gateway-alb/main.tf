...
# Keep this rule separate from the legacy rules above. Loop deployments can
# move their existing one-rule-per-resource state here without replacement.
resource "aws_vpc_security_group_ingress_rule" "gateway_alb_ingress_from_loop_runtime_security_groups" {
  for_each = var.loop_runtime_security_groups

...
variable "health_check_grace_period_seconds" {
  type        = number
  description = "Grace period (seconds) before failed health checks can affect ECS deployment."
  default     = 60
...
variable "enable_deployment_circuit_breaker" {
  type        = bool
  description = "Enable ECS deployment circuit breaker for automatic failure detection."
  default     = true
}

variable "deployment_circuit_breaker_rollback" {
  type        = bool
  description = "Roll back failed deployments automatically when circuit breaker is enabled."
  default     = true
}

variable "scale_in_cooldown" {
  type        = number
  description = "Scale-in cooldown in seconds for ECS target tracking policies."
  default     = 300

  validation {
    condition     = var.scale_in_cooldown >= 0
    error_message = "scale_in_cooldown must be >= 0."
  }
}

variable "scale_out_cooldown" {
  type        = number
  description = "Scale-out cooldown in seconds for ECS target tracking policies."
  default     = 60

  validation {
    condition     = var.scale_out_cooldown >= 0
    error_message = "scale_out_cooldown must be >= 0."
  }
}

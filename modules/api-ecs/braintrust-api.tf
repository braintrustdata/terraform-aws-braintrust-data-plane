...
resource "aws_ecs_service" "braintrust_api" {
  ...
  load_balancer {
    target_group_arn = var.target_group_arns["braintrust_api"]
    container_name   = "api"
    container_port   = 8000
  }
  ...
  depends_on = [terraform_data.brainstore_deployment, terraform_data.alb_http_listener]
...
resource "aws_appautoscaling_policy" "braintrust_api_event_loop_target" {
  name               = "${var.deployment_name}-braintrust-api-event-loop-target"
  policy_type        = "TargetTrackingScaling"
  resource_id        = aws_appautoscaling_target.braintrust_api.resource_id
  scalable_dimension = aws_appautoscaling_target.braintrust_api.scalable_dimension
  service_namespace  = aws_appautoscaling_target.braintrust_api.service_namespace
  ...
  target_tracking_scaling_policy_configuration {
    customized_metric_specification {
      ...
      dimensions {
        name  = "ServiceName"
        value = local.braintrust_api_name
      }

...
CloudFront rejects VPC origin create/update if the origin ALB spans an unsupported AZ (e.g. `use1-az3`), even when other ALB AZs are fine. This module filters ALB (+ matching ECS task) subnets to supported AZs for `ApiEcsOrigin`, private gateway origins, and Loop runtime (`enable_loop_runtime`). VPC AZ defaults stay on `aws_availability_zones.available` so existing `create_vpc` stacks do not ForceNew subnets/NAT on upgrade; greenfield VPCs may still contain a banned AZ, but CloudFront-facing ALBs simply omit it. In regions with an exclusion, those ALB-backed services therefore run across **2 AZs instead of 3** (resilience tradeoff for origin creatability).
